...
object BackupCodec {
    ...
    fun decode(json: String): BackupPayload {
        try {
            ...
            if (root.optInt("schemaVersion", -1) != SCHEMA_VERSION) throw BackupFormatException(BackupFormatException.Reason.UNSUPPORTED_VERSION)
            ...
            for (listIndex in 0 until listArray.length()) {
                ...
                for (itemIndex in 0 until sourceItems.length()) {
                    ...
                    items += BackupItem(
                        ...
                        category = runCatching { WatchCategory.valueOf(item.getString("category")) }
                            .getOrElse { throw BackupFormatException(BackupFormatException.Reason.INVALID_DATA) },
    ...
    fun validate(payload: BackupPayload) {
        ...
        payload.lists.forEach { list ->
            ...
            list.items.forEach { item ->
                ...
                if (item.review.length > 500 || item.rating != null && item.rating !in 1..10) invalid()
